...
class BaseResponse:
    def send_response(self):
        ...
        return Response(self.__dict__, status=code)


class BadRequestResponse(BaseResponse):
    def __init__(self, result=None, status=False, current_datetime=datetime.now(), code=st.HTTP_400_BAD_REQUEST, **kwargs):
        self.result = result
        self.status = status
...
class NotFoundResponse(BaseResponse):
    def __init__(self, result=None, status=False, current_datetime=datetime.now(), code=st.HTTP_404_NOT_FOUND):
        self.result = result
        self.status = status
